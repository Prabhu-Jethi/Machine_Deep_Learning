...
def reciprocal_rank_fusion(chunk_lists, k=60, verbose=True):
    if verbose:
        ...
        print(f"\n k = {k}")
        print(f"\nCalculating RRF score..")

    rrf_scores = defaultdict(float) ## stores {chunk_content: rrf_scores}   
    all_unique_chunks = {}  ## Stores {chunk_content: actual_chunk_objects}
